...
def populate_dataframes(nodes_df, edges_df, num_layers=4, features_per_layer=8):
    color_map = plt.cm.get_cmap("rainbow")
    
    # Precompute the number of features for each layer
    num_features_per_layer = [random.randint(3, features_per_layer) for _ in range(num_layers)]
